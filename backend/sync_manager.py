...
class SyncManager:
    ...
    def sync_uc_with_cookie(self, cookie: str, account_name: str = None, account_id: int = 0) -> dict:
        """使用指定Cookie执行UC网盘同步"""
        if not cookie or len(cookie) < 50:
            return {'success': False, 'message': 'Cookie未配置或格式不正确'}
        ...
        try:
            ...
            # 确定source格式：如果有账号名称，使用"uc:账号名称"
            source = 'uc'
            if account_name and account_name.strip():
                source = f"uc:{account_name.strip()}"
            ...
            for share_data in shares:
                try:
                    ...
                    # 设置网盘端share_id
                    uc_share_id = share_data.get('share_id') or share_data.get('id') or ''
                    if uc_share_id:
                        share_info['share_id'] = str(uc_share_id)
    ...
    def sync_with_cookie(self, platform: str, cookie: str, account_name: str = None, account_id: int = 0) -> dict:
        ...
        if platform == 'quark':
            ...
            return self.sync_uc_with_cookie(cookie, account_name, account_id)
        else:
